...
function main()
    ...
    if toml_path === nothing
        error("Usage: julia MiriamDocumentEntry.jl path/to/config.toml")
        flush(stderr)
    end
    MiriamDocumentRunner.run_MiriamDocument(toml_path); flush(stdout); flush(stderr)
end
