...
Compute bias-aware bootstrap estimates of cumulants using reweighted trace data.

This function computes bootstrap-resampled cumulants (cond, sus, skew, kurt, bind) from multiple estimators,
namely: original (`ORG`), labeled (`LB`), `P1` (bias-corrected), `P2` (weighted blend of `LB` and `P1`),
nbP1 (`P1` with `TRP` = 100%), and nbP2 (`P2` with nbP1 replacing `P1`).

Each trace entry is reweighted by the corresponding weight field `w` from `ReweightingBundle.ReweightingSolverBundle`.
...
- `trMi_all_bundle`: Trace data bundle; `trMi_all_bundle[i][j]` is the j-th configuration in solver i.
...
- `idx_lb`: Optional bootstrap indices for labeled (`LB`) data.
- `idx_bc`: Optional indices for bias correction subset (used in `P1`).
- `idx_ul`: Optional indices for unlabeled data (used in `P1`/`P2`).
...
- If provided, `opt_blk_size` can override the default `blk_size` on a per-estimator basis (e.g., `:lb`, `:tr`, etc.).
"""
function compute_cumulants_bundle(
    ...
    @inbounds for i in 1:N_bs
        ...
        if !has_lb
            ...
            if w == 0.0
                ...
                Q1 = Q_mean_Y_lb[1,i] / w; Q2 = Q_mean_Y_lb[2,i] / w
                Q3 = Q_mean_Y_lb[3,i] / w; Q4 = Q_mean_Y_lb[4,i] / w
                s, sk, k, b = Cumulants.calc_cumulants(float(V), Q1, Q2, Q3, Q4, jobid)
                cond_lb_bs[i] = Q1 / V;  susp_lb_bs[i] = s;  skew_lb_bs[i] = sk;  kurt_lb_bs[i] = k;  bind_lb_bs[i] = b
            end
        end
        ...
        if !has_bc
            ...
            if w == 0.0
                ...
                Q1 = Q_mean_Y_bc[1,i] / w; Q2 = Q_mean_Y_bc[2,i] / w
                Q3 = Q_mean_Y_bc[3,i] / w; Q4 = Q_mean_Y_bc[4,i] / w
                s, sk, k, b = Cumulants.calc_cumulants(float(V), Q1, Q2, Q3, Q4, jobid)
                cond_bc_bs[i] = Q1 / V;  susp_bc_bs[i] = s;  skew_bc_bs[i] = sk;  kurt_bc_bs[i] = k;  bind_bc_bs[i] = b
            end
        end
        ...
        if !has_bc
            ...
            if w == 0.0
                ...
                Q1 = Q_mean_YP_bc[1,i] / w; Q2 = Q_mean_YP_bc[2,i] / w
                Q3 = Q_mean_YP_bc[3,i] / w; Q4 = Q_mean_YP_bc[4,i] / w
                s, sk, k, b = Cumulants.calc_cumulants(float(V), Q1, Q2, Q3, Q4, jobid)
                cond_ypbc_bs[i] = Q1 / V;  susp_ypbc_bs[i] = s;  skew_ypbc_bs[i] = sk;  kurt_ypbc_bs[i] = k;  bind_ypbc_bs[i] = b
            end
        end
        ...
        if !has_ul
            ...
            if w == 0.0
                ...
                Q1 = Q_mean_Y_ul[1,i] / w; Q2 = Q_mean_Y_ul[2,i] / w
                Q3 = Q_mean_Y_ul[3,i] / w; Q4 = Q_mean_Y_ul[4,i] / w
                s, sk, k, b = Cumulants.calc_cumulants(float(V), Q1, Q2, Q3, Q4, jobid)
                cond_ul_bs[i] = Q1 / V;  susp_ul_bs[i] = s;  skew_ul_bs[i] = sk;  kurt_ul_bs[i] = k;  bind_ul_bs[i] = b
            end
        end
        ...
        if !has_ul
            ...
            if w == 0.0
                ...
                Q1 = Q_mean_YP_ul[1,i] / w; Q2 = Q_mean_YP_ul[2,i] / w
                Q3 = Q_mean_YP_ul[3,i] / w; Q4 = Q_mean_YP_ul[4,i] / w
                s, sk, k, b = Cumulants.calc_cumulants(float(V), Q1, Q2, Q3, Q4, jobid)
                cond_ypul_bs[i] = Q1 / V;  susp_ypul_bs[i] = s;  skew_ypul_bs[i] = sk;  kurt_ypul_bs[i] = k;  bind_ypul_bs[i] = b
            end
        end
    end
    ...
    @inbounds for i in 1:N_bs
        if N_tr == 0
            ...
            # YmYP = BC − YP_BC (at cumulant level)
            ...
            pass_through = (N_tr == N_lb)
            cond_P1 = pass_through ? cond_ypul_bs[i] : (cond_ypul_bs[i] + cond_YmYP)
            susp_P1 = pass_through ? susp_ypul_bs[i] : (susp_ypul_bs[i] + susp_YmYP)
            skew_P1 = pass_through ? skew_ypul_bs[i] : (skew_ypul_bs[i] + skew_YmYP)
            kurt_P1 = pass_through ? kurt_ypul_bs[i] : (kurt_ypul_bs[i] + kurt_YmYP)
            bind_P1 = pass_through ? bind_ypul_bs[i] : (bind_ypul_bs[i] + bind_YmYP)
    ...
    cumulants_LB_bs   = [cond_lb_bs,  susp_lb_bs,  skew_lb_bs,  kurt_lb_bs,  bind_lb_bs ]
    ...
    return cumulants_OG_bs, cumulants_P1_bs, cumulants_P2_bs, cumulants_LB_bs, N_tr, w_lb, w_ul, plan_bundle
end