# =============================================================================
# src/Miriam/CumulantsBundle/compute_moments_bundle_raw.jl
#
...
Compute bootstrap estimates of moments ``Q_{n} \\; (n=1,2,3,4)`` for single ensemble.
This mirrors the structure of
[`compute_cumulants_bundle_raw`](@ref) but returns moment components instead of cumulants.
...
- `trMi_all_bundle::Vector{Vector{Vector{T}}}`: For each array in the bundle, a vector of
  per-configuration trace vectors (e.g., length-5 `trMi` rows). Each inner `Vector{T}`
  must be consumable by [`Cumulants.calc_Q`](@ref).
- [`ens_bundle::Ensemble.EnsembleArrayBundle{T}`](@ref Ensemble.EnsembleArrayBundle): Bundle containing the ensemble arrays.
...
- `(moments_OG_bs, moments_P1_bs, moments_P2_bs, idx_bundle)` where:
  ...
  - `moments_P2_bs::Vector{Vector{Float64}}` = `[Q1_P2_bs,  Q2_P2_bs,  Q3_P2_bs,  Q4_P2_bs ]`
    Each `Qk_*_bs` is a length-`N_bs` vector of bootstrap means.
  - `idx_bundle::Dict{Symbol, Union{Nothing, Matrix{Int}}}`:
    `:all`, `:lb`, `:bc`, `:ul` → the index plans used for each subset (or `nothing`).

# Notes
- Subset splitting uses [`Deborah.Miriam.CumulantsBundleUtils.split_Q_full`](@ref) on the tuple
  `(Q1,Q2,Q3,Q4)` per array.
- `P1` is constructed analogously to the cumulant pipeline:
  `Y_P1 = (N_tr == N_lb) ? YP_UL : (YP_UL + (Y_BC - YP_BC))`, applied component-wise to `Q1,Q2,Q3,Q4`.
...
"""
function compute_moments_bundle_raw(
    N_bs::Int,
    blk_size::Int,
    method::String,
    trMi_all_bundle::Vector{Vector{Vector{T}}},
    ...
    # === Compute Q1..Q4 for each array in the bundle (no reweighting factor) ===
    ...
    Q1_ORG_bs = Vector{Float64}(undef, N_bs)
    Q2_ORG_bs = Vector{Float64}(undef, N_bs)
    Q3_ORG_bs = Vector{Float64}(undef, N_bs)
    Q4_ORG_bs = Vector{Float64}(undef, N_bs)
    ...
    @inbounds for i in 1:N_bs
        # ORG = means over ALL
        Q1_ORG = Q_mean_Y_ORG[1, i]
        Q2_ORG = Q_mean_Y_ORG[2, i]
        Q3_ORG = Q_mean_Y_ORG[3, i]
        Q4_ORG = Q_mean_Y_ORG[4, i]
        ...
        if N_tr == 0
            ...
            Q1_P1 = Q_mean_Y_lb[1, i]
            Q2_P1 = Q_mean_Y_lb[2, i]
            Q3_P1 = Q_mean_Y_lb[3, i]
            Q4_P1 = Q_mean_Y_lb[4, i]
        else
            ΔQ1 = Q_mean_Y_bc[1, i] - Q_mean_YP_bc[1, i]
            ΔQ2 = Q_mean_Y_bc[2, i] - Q_mean_YP_bc[2, i]
            ΔQ3 = Q_mean_Y_bc[3, i] - Q_mean_YP_bc[3, i]
            ΔQ4 = Q_mean_Y_bc[4, i] - Q_mean_YP_bc[4, i]

            if N_tr == N_lb
                # Full pass-through
                Q1_P1 = Q_mean_YP_ul[1, i]
                Q2_P1 = Q_mean_YP_ul[2, i]
                Q3_P1 = Q_mean_YP_ul[3, i]
                Q4_P1 = Q_mean_YP_ul[4, i]
            else
                Q1_P1 = Q_mean_YP_ul[1, i] + ΔQ1
                Q2_P1 = Q_mean_YP_ul[2, i] + ΔQ2
                Q3_P1 = Q_mean_YP_ul[3, i] + ΔQ3
                Q4_P1 = Q_mean_YP_ul[4, i] + ΔQ4
            end
        end
        ...
        # P2 = w_lb * LB + w_ul * P1
        Q1_P2 = w_lb * Q_mean_Y_lb[1, i] + w_ul * Q1_P1
        Q2_P2 = w_lb * Q_mean_Y_lb[2, i] + w_ul * Q2_P1
        Q3_P2 = w_lb * Q_mean_Y_lb[3, i] + w_ul * Q3_P1
        Q4_P2 = w_lb * Q_mean_Y_lb[4, i] + w_ul * Q4_P1
    ...
    # === Pack results ===
    moments_OG_bs = [Q1_ORG_bs, Q2_ORG_bs, Q3_ORG_bs, Q4_ORG_bs]
    moments_P1_bs = [Q1_P1_bs,  Q2_P1_bs,  Q3_P1_bs,  Q4_P1_bs ]
    moments_P2_bs = [Q1_P2_bs,  Q2_P2_bs,  Q3_P2_bs,  Q4_P2_bs ]
    ...
    return moments_OG_bs, moments_P1_bs, moments_P2_bs, idx_bundle
end