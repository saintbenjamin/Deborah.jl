...
# ----------------------------------------------------------------------------
using Revise
import Deborah.EstherThreads.EstherThreadsRunner; flush(stdout); flush(stderr)
...
function main()
    ...
    if toml_path === nothing
        error("Usage: julia EstherThreads.jl path/to/config.toml")
        flush(stderr)
    end
    EstherThreadsRunner.run_EstherThreads(toml_path); flush(stdout); flush(stderr)
end
